globalThis.__timing__.logStart('Load chunks/build/partnership-4M2D-o7l');import { _ as __nuxt_component_0 } from './SiteHeader-D9ODwCeG.mjs';
import { defineComponent, withAsyncContext, reactive, ref, computed, mergeProps, unref, useSSRContext } from 'vue';
import { ssrRenderAttrs, ssrRenderComponent, ssrRenderList, ssrIncludeBooleanAttr, ssrLooseContain, ssrRenderAttr, ssrInterpolate, ssrRenderClass, ssrLooseEqual } from 'vue/server-renderer';
import { g as getContactUsPage, l as leadBudgetOptions } from './api-OwmsrwqD.mjs';
import { h as usePageSeo, i as useRoute } from './server.mjs';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    usePageSeo({
      ...
      description: "Start a partnership with TWF Agency across media strategy, performance marketing, creative, CRM, commerce, analytics, and technology services.",
      path: "/partnership",
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`</label><label class="${ssrRenderClass(["partnership-field", { "has-error": hasFieldError("budget") }])}"><span>Budget</span><select name="budget" required${ssrRenderAttr("aria-invalid", hasFieldError("budget"))}${ssrRenderAttr("aria-describedby", unref(fieldErrors).budget ? "partnership-budget-error" : void 0)}><option value="" disabled${ssrIncludeBooleanAttr(Array.isArray(unref(form).budget) ? ssrLooseContain(unref(form).budget, "") : ssrLooseEqual(unref(form).budget, "")) ? " selected" : ""}>Select budget</option><!--[-->`);
      ssrRenderList(unref(leadBudgetOptions), (option) => {
        _push(`<option${ssrRenderAttr("value", option)}${ssrIncludeBooleanAttr(Array.isArray(unref(form).budget) ? ssrLooseContain(unref(form).budget, option) : ssrLooseEqual(unref(form).budget, option)) ? " selected" : ""}>${ssrInterpolate(option)}</option>`);
      });
      _push(`<!--]--></select>`);
...
export { _sfc_main as default };;globalThis.__timing__.logEnd('Load chunks/build/partnership-4M2D-o7l');
//# sourceMappingURL=partnership-4M2D-o7l.mjs.map
